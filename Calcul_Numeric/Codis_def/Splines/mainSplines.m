...
figure(2)
for i=1:length(x)
    x1= [1:length(x)]; y1 = zeros(1, length(x)); y1(i) = 1;
    dS = zeros(length(x));
...
for i=1:length(x)
    x1= [1:length(x)]; y1 = zeros(1, length(x));
...
for i=1:length(x)
    x1 = [1:length(x)]; y1 = zeros(1, length(x)); y1(i) = 1;
    d2S = calculaCurvaturesSplineNatural(x1, y1);
    [xS2,yS2,coeficients]=dibuixaSplineCubic(x1,y1,[],d2S);
    subplot(length(x),1,i), plot(xS2,yS2,'r-',x1,y1,'ko','LineWidth',2)
